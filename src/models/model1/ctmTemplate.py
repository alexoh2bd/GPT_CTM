...
class ContinuousThoughtMachine(nn.Module):
    ...
    def __init__(
        ...
        self.memory_length = memory_length
        self.backbone = GPT.from_pretrained("gpt2")
        ...
        self.synch_representation_size_action = (
            self.calculate_synch_representation_size(self.n_synch_action)
        )
        self.synch_representation_size_out = self.calculate_synch_representation_size(
            self.n_synch_out
        )
        ...
        if self.synch_representation_size_action:  # if not zero
            self.set_synchronisation_parameters(
                "action", self.n_synch_action, n_random_pairing_self
            )
        self.set_synchronisation_parameters(
            "out", self.n_synch_out, n_random_pairing_self
        )
    ...
    def compute_synchronisation(
        self, activated_state, decay_alpha, decay_beta, r, synch_type
    ):
        """
    ...
    def get_positional_embedding(self):
        ...
        """
        return torch.arange(0, T, dtype=torch.long, device=x.device)
    ...
    def get_synapses(self, synapse_depth, d_model, dropout):
        ...
        if synapse_depth == 1:
            ...
            return SynapseUNET(
                d_model, synapse_depth, 16, dropout
            )  # hard-coded minimum width of 16; future work TODO.

    def set_synchronisation_parameters(
        self, synch_type: str, n_synch: int, n_random_pairing_self: int = 0
    ):
        """
    ...
    def initialize_left_right_neurons(
        self, synch_type, d_model, n_synch, n_random_pairing_self=0
    ):
        """
        ...
        if self.neuron_select_type == "first-last":
            ...
            neuron_indices_right = torch.concatenate(
                (
                    ...
                    torch.from_numpy(
                        np.random.choice(
                            np.arange(d_model), size=n_synch - n_random_pairing_self
                        )
                    ),
                )
            )
    ...
    def verify_args(self):
        ...
        if self.backbone_type == "none" and self.positional_embedding_type != "none":
            raise AssertionError(
                "There should be no positional embedding if there is no backbone."
            )
